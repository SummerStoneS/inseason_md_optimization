#####2020.6.30
#####added global variable pass_days to represent the time interval from the planned markdown
import sys
import os
import pandas as pd
import datetime
...
########2. load latest version of feature data
df_old = pd.read_csv(os.path.join(paths.source, sophie_features_src))
# load raw data from Step1(merged data from db: transaction + attribute + inventory 1-day prior sales_date)
...
df_sales_amt=df.sort_values(axis = 0,ascending = [True,True,True,False],by = ['sales_date','Style_Number','inseason_flag1','sales_amt'])
df_sales_amt=df_sales_amt.drop_duplicates(['Style_Number','sales_date','inseason_flag1'], 'first', inplace=False)[['sales_date','Style_Number','Gender Group','GC_Category','season1','inseason_flag1','SEASON','Subcategory']]



df=df.groupby(['sales_date','Style_Number','inseason_flag1']).agg({'sales_qty':'sum','sales_amt':'sum','MSRP':pd.Series.median,'inv_qty':'sum','md':'mean'}).reset_index()

df=pd.merge(df, df_sales_amt, on = ['Style_Number','sales_date','inseason_flag1'], how = 'left')
...
for i in range(len(season_list)):
    ...
    season_number_i=df.query("sales_date<=@season_end").groupby(['Style_Number','inseason_flag1']).agg({'season1':pd.Series.nunique}).reset_index()
...
df = pd.merge(df, season_number, on = ['Style_Number','season1','inseason_flag1'], how = 'left')
...
for i in range(len(season_list)):
    print(season_list[i])
    season_end=pd.to_datetime(season_end_list[season_end_list["Season"]==season_list[i]]["DATE"].iloc[0])
    inseason_number_i=df.query("(sales_date<=@season_end)&(inseason_flag1=='Y')").groupby(['Style_Number','inseason_flag1']).agg({'season1':pd.Series.nunique}).reset_index()
...
df = pd.merge(df, inseason_number, on = ['Style_Number','season1','inseason_flag1'], how = 'left')
...
for i in range(len(season_list)):
    ...
    season_end=pd.to_datetime(season_end_list[season_end_list["Season"]==season_list[i]]["DATE"].iloc[0])
    inseason_1st_day_i=df.query("(sales_date<=@season_end)&(sales_date>=@season_start)")[['season1','Style_Number','sales_date','inseason_flag1']]
    inseason_1st_day_i=inseason_1st_day_i.drop_duplicates(['Style_Number','inseason_flag1'], 'first', inplace=False)[['Style_Number','sales_date','season1','inseason_flag1']]
...
df = pd.merge(df, inseason_1st_day, on = ['Style_Number','season1','inseason_flag1'], how = 'left')
...
temp=df.copy()
temp=temp.drop_duplicates(['Style_Number','inseason_flag1'], 'first', inplace=False)
temp.rename(columns = {'sales_date': 'first_sales_date'}, inplace = True)
temp=temp[['first_sales_date','Style_Number','inseason_flag1']]
df = pd.merge(df, temp, on = ['Style_Number','inseason_flag1'], how = 'left')
...
for i in range(len(season_list)):
    ...
    first_14d_i=df.query("(sales_date<=@a14)&(sales_date>=@a)")[['season1','Style_Number','inseason_flag1','sales_qty','md','inseason_1st_day']]
    ...
    first_14d_i=first_14d_i.groupby(['Style_Number','season1','inseason_flag1']).agg({'sales_qty':'sum','md':'mean'}).reset_index()
...
df = pd.merge(df, first_14d, on = ['Style_Number','season1','inseason_flag1'], how = 'left')
...
for i in range(len(season_list)):
    ...
    md_first_day_i=df.query("(sales_date<=@season_end)&(sales_date>=@season_start)").drop_duplicates(['Style_Number','inseason_flag1','md'], 'first', inplace=False)[['sales_date','Style_Number','inseason_flag1','md','season1']]
    md_first_day_i.rename(columns = {'sales_date': 'this_md_first_day'}, inplace = True)
    md_first_day=md_first_day.append(md_first_day_i)


df = pd.merge(df, md_first_day, on = ['Style_Number','md','season1','inseason_flag1'], how = 'left')
...
# 真正需要运算的日期
    inseason_APS_this_md_i=df.query("(sales_date<=@season_end)&(sales_date>=@season_start)&(sales_date>=@code_start_date)")[['sales_date','Style_Number','sales_qty','season1','md','inseason_APS_this_md','inseason_flag1']]

# 取数的data
    data=df.query("(sales_date<=@season_end)&(sales_date>=@season_start)")[['sales_date','Style_Number','sales_qty','season1','md','inseason_APS_this_md','inseason_flag1']]
    ...
    for x in range(0,inseason_APS_this_md_i.shape[0]): 
        ...
        x_flag=inseason_APS_this_md_i.iloc[x,6]
        x_style_number=inseason_APS_this_md_i.iloc[x,1]
        temp=data.query("(sales_date<=@x_date1)&(md==@x_md)&(Style_Number==@x_style_number)&(inseason_flag1==@x_flag)")
...
inseason_APS_this_md=inseason_APS_this_md[['Style_Number','inseason_flag1','sales_date','md','inseason_APS_this_md']]
df = pd.merge(df, inseason_APS_this_md, on = ['Style_Number','inseason_flag1','sales_date','md'], how = 'left')
...
def cal_on_market_days(a,b):
    ...
    data=df.query("(sales_date<=@b)&(sales_date>=@a)")[['sales_date','Style_Number','num_days_on_mkt','num_days_on_mkt_inseason']].drop_duplicates(['Style_Number','sales_date'], 'first', inplace=False)
    
    for x in range(0,data.shape[0]): 
        x_date1=data.iloc[x,0]
        x_style_number=data.iloc[x,1]
        temp=df.query("(sales_date<=@x_date1)&(Style_Number==@x_style_number)")
...
df = pd.merge(df, num_days_on_mkt, on = ['Style_Number','sales_date'], how = 'left')
...
def cal_AUR_inv_advantage(a,b):
    ...
    data=df.query("(sales_date<=@b)&(sales_date>=@a)")[['sales_date','Style_Number','inv_qty','group','md_price','inv_adv','AUR_inv_adv','md','md_inv_adv','comp_price_low','comp_price_high','inseason_flag1']]
    ...
    for x in range(0,data.shape[0]): 
        ...
        x_style_number=data.iloc[x,1]
        x_flag=data.iloc[x,11]
        
        data2=data.query("(sales_date==@x_date1)&(Style_Number==@x_style_number)&(inseason_flag1==@x_flag)")
...
inv_adv=inv_adv[['sales_date','Style_Number','inv_adv','AUR_inv_adv','md_inv_adv','inseason_flag1']]

df=df.drop(["inv_adv","AUR_inv_adv","md_inv_adv"],axis=1)
df = pd.merge(df, inv_adv, on = ['Style_Number','sales_date','inseason_flag1'], how = 'left')
...
def cal_last_md(a,b):
    ...
    data=df.query("(sales_date<=@b)&(sales_date>=@a)")[['sales_date','Style_Number','md','aps_last_md','interval_after_last_md','inv_qty','group','md_price','inv_adv_last_md','AUR_inv_adv_last_md','md_inv_adv_last_md','sales_qty','comp_price_low','comp_price_high','inseason_flag1']].drop_duplicates(['Style_Number','sales_date','inseason_flag1'], 'first', inplace=False)
    ...
    for x in range(0,data.shape[0]): 
        ...
        x_flag=data.iloc[x,14]

        temp=df.query("(sales_date<=@x_date1)&(md==@x_md)&(Style_Number==@x_style_number)&(inseason_flag1==@x_flag)")

        #delete retail moments or not based on excl_rm
        temp=temp[~temp.sales_date.isin(rm_dates)]

        temp=temp.drop_duplicates(['Style_Number','md','inseason_flag1'], 'last', inplace=False)
...
last_md=last_md[['sales_date','Style_Number','md','inseason_flag1','aps_last_md','interval_after_last_md','inv_adv_last_md','AUR_inv_adv_last_md','md_inv_adv_last_md']]

df=df.drop(["aps_last_md","interval_after_last_md","inv_adv_last_md","AUR_inv_adv_last_md","md_inv_adv_last_md"],axis=1)
df = pd.merge(df, last_md, on = ['Style_Number','sales_date','md','inseason_flag1'], how = 'left')
...
for i in range(len(season_list)):
    ...
    data=df.query("(sales_date<=@season_end)&(sales_date>=@season_start)")[['sales_date','Style_Number','md','num_days_this_md_inseason','num_days_comp_this_md_inseason','group','comp_price_low','comp_price_high','md_price','inseason_flag1']]
    ...
    for x in range(0,data.shape[0]): 
        ...
        x_flag=data.iloc[x,9]        
        temp=data.query("(sales_date<=@x_date1)&(md==@x_md)&(Style_Number==@x_style_number)&(inseason_flag1==@x_flag)")
        data.iloc[x,3]=temp.shape[0]
        temp2=data.query("(sales_date<@x_date1)&(md==@x_md)&(group==@x_group)&(Style_Number!=@x_style_number)&(md_price>=@x_low)&(md_price<=@x_high)")
...
days_under_this_markdown=days_under_this_markdown[['sales_date','Style_Number','inseason_flag1','md','num_days_this_md_inseason','num_days_comp_this_md_inseason']]

df=df.drop(["num_days_this_md_inseason","num_days_comp_this_md_inseason"],axis=1)
df = pd.merge(df, days_under_this_markdown, on = ['Style_Number','inseason_flag1','sales_date','md'], how = 'left')



###########replace the old feature data

df_new=df.query("(sales_date>=@code_start_date)&(sales_date<=@code_end_date)")

df_old['sales_date'] = pd.to_datetime(df_old['sales_date'])

df_old=df_old.query("(sales_date<@code_start_date)|(sales_date>@code_end_date)")[['Style_Number', 'sales_date', 'inseason_flag1',
...
df=df_old.append(df_new)[['Style_Number', 'sales_date', 'inseason_flag1',
       'sales_qty', 'sales_amt', 'MSRP', 'inv_qty', 'Gender Group',
       'GC_Category', 'season1', 'SEASON', 'Subcategory', 'md_price', 'md',
       'comp_price_low', 'comp_price_high', 'num_seasons', 'num_inseasons',
       'group', 'num_styles_in_group', 'num_styles_on_sale_in_group',
...
df=df.sort_values(axis = 0,ascending = [True,True,True,False],by = ['sales_date','Style_Number','inseason_flag1','sales_amt'])

df.to_csv(os.path.join(paths.source, sophie_features_src))
